...
4. **Execute reproducibly.** Prefer the bundled standard-library scripts for their matching method. Use local, bounded inputs; record script path, version, command, hash, formula, input refs, assumptions, and rounding. JSON/CSV output is transient unless the user explicitly requests a registered derived export.
...
Persist a requested JSON/CSV export only as `creation_mode: derived`, `semantic_authority: none`, with path, schema, hash, provenance, and a reference from this artifact. An export can feed a chart or audit but cannot be the only semantic support of a synthesis or report block. This stage never writes workflow state or the artifact index.
