# TypeScript / Next.js / tRPC / Data-layer Checklist
...
**This project:** Next.js 16 (App Router) · React 19 · tRPC v11 · ORM/data layer resolved from the repo (`package.json`, schema files) · Zod 4 · better-auth · TypeScript via **tsgo** (`pnpm typecheck`; not migrated to TS v7) · Biome (`pnpm check`) · vitest + Playwright.
...
*(This section applies when the repo's ORM is Prisma — resolve the actual ORM from `package.json` and schema files first. For another ORM — Drizzle, Kysely, etc. — apply the equivalent checks with that ORM's idioms: schema/type/Zod sync, transactions, query efficiency, parameterization, enum sync, data-access boundaries, nullable normalization, decimal handling, migration hygiene.)*
