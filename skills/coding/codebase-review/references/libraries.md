...
> **Not in this project (do not flag their absence):** BullMQ/Redis — there is no queue or background-job infrastructure yet; work that would need one is done inline or deferred by design. MUI — the UI layer is shadcn/ui + Radix + Tailwind. The ORM is resolved from the repo — apply the matching ORM section and skip the others.
...
*(Deep DB items are in `t3-stack.md` under `PRSM`; this section is for library-idiom specifics. Applies when the repo's ORM is Prisma — resolve the actual ORM from the repo first; for another ORM, apply the equivalent idioms.)*
...
- **LIB-AUTH-5 Schema alignment** — the database models backing better-auth match what the installed version and its enabled plugins expect; schema drift here surfaces as runtime auth failures.
...
- **LIB-MISC-4 Test-layer fit** — pure business logic is extracted and unit-tested (the project's established pattern: assemblers, diagnostics, filters tested with hand-built fixtures), rather than only reachable through an e2e run. A rule that could be a vitest test but is only covered by Playwright is a finding. Extraction is the right pattern when the logic is genuinely independent; if the extracted pieces are tested but the real wiring where the bugs live is not, or the extraction scatters one concept across callers, that's an `ARCH-4`/`ARCH-5` finding instead (see `generic-standards.md`).
