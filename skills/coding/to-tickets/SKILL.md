...
Use this skill only when work needs distribution, a tracker, multiple executors, or cold-start continuity. Otherwise continue directly to `implement`.
...
Tickets are durable `Working` execution records. `implement` updates each original ticket with status, change summary, acceptance coverage, tests, mini-review result, deviations, and follow-ups. It does not create a parallel durable summary.
