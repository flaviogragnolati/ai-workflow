"""Calculate a transparent weighted candidate matrix with sensitivity analysis.
...
@dataclass(frozen=True)
class CandidateRow:
    """Validated ratings, gates, and qualitative context for one candidate."""
...
def load_criteria(raw_path: str) -> list[Criterion]:
    ...
    if len(raw_criteria) > MAX_CRITERIA:
        raise CliError(f"criteria count is {len(raw_criteria)}; limit is {MAX_CRITERIA}")
    ...
    for index, raw in enumerate(raw_criteria):
        ...
        if name.startswith(GATE_PREFIX):
            raise CliError(
                f"{label}.name must not start with {GATE_PREFIX!r}; gates stay outside the score"
            )
        if name in seen:
            raise CliError(f"duplicate criterion name: {name}")
        seen.add(name)
        description = require_text(raw.get("description"), f"{label}.description", maximum=500)
...
def _parse_bounded_score(raw: str | None, *, label: str, criterion: Criterion) -> float:
    ...
    if not criterion.minimum <= score <= criterion.maximum:
        raise CliError(f"{label}={score} is outside [{criterion.minimum}, {criterion.maximum}]")
    return score


def load_scores(raw_path: str, criteria: Sequence[Criterion]) -> list[CandidateRow]:
    """Load bounded CSV ratings and preserve gate and qualitative columns."""
    ...
    required = {"candidate_id", "qualitative_review", "uncertainties"}
    ...
    for criterion in criteria:
        ...
        if has_low != has_high:
            raise CliError(
                f"uncertainty columns for {criterion.name} must include both {low_name} and {high_name}"
            )
        if has_low:
            interval_columns.update({low_name, high_name})

    numeric_columns = {criterion.name for criterion in criteria} | interval_columns
    gate_columns = [name for name in fieldnames if name.startswith(GATE_PREFIX)]
    qualitative_columns = [
        name
        for name in fieldnames
        if name not in numeric_columns and name not in gate_columns and name != "candidate_id"
    ]
    ...
    for row_index, raw_row in enumerate(reader, start=2):
        ...
        candidate_id = require_identifier(raw_row.get("candidate_id"), f"row {row_index}.candidate_id")
        if candidate_id in seen_ids:
            raise CliError(f"duplicate candidate_id in scores CSV: {candidate_id}")
        ...
        for criterion in criteria:
            ...
            if low_name in interval_columns:
                ...
                if low_blank != high_blank:
                    raise CliError(
                        f"row {row_index} must provide both {low_name} and {high_name}, or leave both blank"
                    )
                if low_blank:
                    low = high = score
                else:
            ...
            if not low <= score <= high:
                raise CliError(f"row {row_index}.{criterion.name} requires low <= score <= high")
...
def _normalized_weights(
    ...
    *,
    ...
    raw = {
        criterion.name: (override[criterion.name] if override is not None else criterion.weight)
        for criterion in criteria
    }
...
def _score(row: CandidateRow, criteria: Sequence[Criterion], weights: Mapping[str, float]) -> float:
    return 100.0 * sum(
...
def _ranks(scores: Mapping[str, float]) -> dict[str, int]:
    """Assign deterministic ordinal ranks; lexical candidate ID breaks ties."""

    order = sorted(scores, key=lambda candidate_id: (-scores[candidate_id], candidate_id))
    return {candidate_id: index + 1 for index, candidate_id in enumerate(order)}
...
def calculate_matrix(
    ...
    *,
    ...
    base_scores = {row.candidate_id: _score(row, criteria, base_weights) for row in rows}
    ...
    if weight_delta > 0:
        for criterion in criteria:
            for sign, factor in (("minus", 1.0 - weight_delta), ("plus", 1.0 + weight_delta)):
                raw_weights = {
                    item.name: (item.weight * factor if item.name == criterion.name else item.weight)
                    for item in criteria
                }
                weights = _normalized_weights(criteria, override=raw_weights)
                scores = {row.candidate_id: _score(row, criteria, weights) for row in rows}
    ...
    for row in sorted(rows, key=lambda item: item.candidate_id):
        ...
        for criterion in criteria:
            ...
            criterion_details[criterion.name] = {
                ...
                "normalized_interval": [rounded(normalized_low), rounded(normalized_high)],
                "normalized_weight": rounded(weight),
                "contribution_to_score": rounded(100.0 * weight * normalized_score),
            }

        sensitivity_scores = [scores[row.candidate_id] for scores in scenario_scores] or [
            base_scores[row.candidate_id]
        ]
        sensitivity_ranks = [ranks[row.candidate_id] for ranks in scenario_ranks] or [
            base_ranks[row.candidate_id]
        ]
        rank_min = min(sensitivity_ranks)
        rank_max = max(sensitivity_ranks)
        if rank_min != rank_max:
            warnings.append(
                f"{row.candidate_id} changes rank from {rank_min} to {rank_max} "
                "under one-at-a-time weight perturbations"
            )
        results.append(
            {
                "candidate_id": row.candidate_id,
                "base_score": rounded(base_scores[row.candidate_id]),
                "base_presentation_rank": base_ranks[row.candidate_id],
                "input_uncertainty_score_interval": [rounded(interval_low), rounded(interval_high)],
    ...
    return {
        ...
        "method": {
            ...
            "formula": "score_i = 100 * sum_j(normalized_weight_j * direction_normalized_rating_ij)",
            "higher_normalization": "(x - minimum) / (maximum - minimum)",
            "lower_normalization": "(maximum - x) / (maximum - minimum)",
            "supplied_weights": {criterion.name: criterion.weight for criterion in criteria},
            "normalized_weights": {name: rounded(value) for name, value in base_weights.items()},
            "criteria": [
                {
                    "name": criterion.name,
            ...
            "gate_columns_excluded_from_score": sorted(
                {name for row in rows for name in row.gates}
            ),
            "weight_sensitivity": {
            ...
            "tie_rule": (
                "Scores are ordered descending for presentation; exact ties are broken by "
                "lexical candidate_id. Rank is not a decision."
            ),
        },
        "results": results,
        "warnings": warnings,
        "decision": None,
        "notice": (
            "No candidate was selected and no conclusion was made. Review qualitative reasons, "
            "uncertainty, dissent, evidence, and the non-compensatory gates before any disposition."
        ),
        "limitations": [
            "The additive model permits compensation across criteria; hard gates stay outside the score.",
            "Input intervals are simultaneous criterion-wise bounds, not calibrated probability intervals.",
            "Weight sensitivity changes one weight at a time and does not cover all plausible weights, criterion dependence, scale uncertainty, or model-form uncertainty.",
            "Ratings and weights are judgments, not measurements.",
        ],
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Calculate a disclosed weighted candidate matrix from bounded CSV ratings, "
            "including input intervals, local weight sensitivity, and preserved gate columns. "
            "No network, model, or automatic selection is used."
        )
    )
    parser.add_argument("scores", help="Input ratings .csv file")
    parser.add_argument("--config", required=True, help="Explicit criteria and weights .json file")
    parser.add_argument(
        "--weight-delta",
        type=bounded_float(0.0, 0.5),
        default=0.10,
        help="Proportional one-at-a-time weight perturbation from 0 through 0.5 (default: 0.10)",
    )
    parser.add_argument("--output", help="Optional result .json file in an existing directory")
    parser.add_argument(
        "--force",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = calculate_matrix(criteria, rows, weight_delta=args.weight_delta)
