...
def _safe_output_target(raw_path: str, suffixes: set[str]) -> Path:
    ...
    if requested.suffix.lower() not in suffixes:
        expected = ", ".join(sorted(suffixes))
        raise CliError(f"output filename must end in one of: {expected}")
...
def write_text(
    ...
    *,
    ...
    """Write text with private permissions and an explicit overwrite decision."""

    target = _safe_output_target(raw_path, suffixes)
...
def write_json(raw_path: str, payload: Any, *, force: bool = False) -> Path:
...
def emit_json(payload: Any, output: str | None, *, force: bool = False) -> Path | None:
    """Print strict JSON or write it to a safe output path."""

    if output is None:
        print(serialize_json(payload), end="")
        return None
