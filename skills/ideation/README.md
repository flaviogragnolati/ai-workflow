...
```mermaid
flowchart TB
    D["One named decision<br/>profile · intent · participation_mode declared first"] --> G["Generate independently<br/>candidates with provenance and assumptions"]
...
**Three dimensions declared before generating anything.** `profile` — `scientific`, `product`, `consulting`, or `general` — selects the vocabulary and candidate kinds; `intent` — `frame-problem`, `generate-options`, `stress-test-options`, or `reopen-after-evidence` — selects the route; `participation_mode` — `facilitated-human`, `solo-assisted`, `asynchronous`, or `agent-only` — sets the facilitation shape and triggers the responsible-AI reference for any AI-assisted or `agent-only` generation. Together they set the rigor the decision earns. An opportunity-discovery sweep is `profile: product` with its own reference.
...
- The session writes three artifacts — `ideation-register` (supporting provenance and raw assessments), `ideation-baseline` (the canonical snapshot), and `ideation-evaluation` (derived calculation only) — all created and kept `Working`; only an adopting root orchestrator registers a version and moves its lifecycle.
