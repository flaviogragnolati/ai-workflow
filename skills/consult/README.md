...
The `consult` group executes an accepted consulting, assessment, training, or managed-service engagement — or the non-software scope of a mixed one — from kickoff to recorded client acceptance. `q-consult-workflow` routes four stages and writes the execution release; the stages own the engagement plan, the assessed current state and its evidence, the intervention design and deliverables, and the acceptance record. Nothing here edits the accepted proposal.
...
```mermaid
flowchart TB
    P["Accepted proposal or engagement agreement<br/>(from Proposal)"] --> W["q-consult-workflow"]
    ...
    W -. "commitment deviation" .-> CC["Proposal change control"]
...
- Execution never edits the accepted proposal; a deviation from scope, price, schedule, deliverables, or criteria is a change request routed to Proposal.
